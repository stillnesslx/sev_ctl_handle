...
TickType_t xTimeToWait;
	...
	for( ;; )
	{
        ...
        com_sdata.sum = 0;
        for(i=0;i<sizeof(struct com_send_data)-2;i++)
        {
            com_sdata.sum += *((unsigned char *)&com_sdata + i);
        }
        vSerialPutString( xPort, (const signed char * ) &com_sdata, sizeof(struct com_send_data)+1 );
